    ...
    <section className="max-sm:py-8 md:py-32 px-4 md:px-8 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-24 gap-8">
          <div className="max-w-3xl">
            ...
            <h2 className="font-heading text-4xl md:text-6xl font-black tracking-tighter text-slate-900 leading-[1.1]">
              Engineered for <br />
              <span className="text-slate-400 font-light italic bg-clip-text bg-gradient-to-r from-slate-400 to-slate-500">
                Performance.
              </span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 auto-rows-min">
          ...
          <BentoCard 
            className="md:col-span-2 lg:col-span-7 lg:row-span-2 bg-white border-slate-100 md:min-h-[400px]"
          ...
          <BentoCard 
            className="md:col-span-1 lg:col-span-5 bg-slate-950 text-white border-white/5 md:min-h-[240px]"
          ...
          <BentoCard 
            className="md:col-span-1 lg:col-span-5 bg-blue-600 text-white border-transparent shadow-2xl shadow-blue-500/10 md:min-h-[240px]"
          ...
          <BentoCard 
            className="md:col-span-2 lg:col-span-12 bg-white border-slate-100 md:min-h-[160px]"
...
const BentoCard: React.FC<BentoCardProps> = ({ 
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`group relative rounded-[2rem] md:rounded-[2.5rem] 
        p-6 md:p-8 lg:p-10 border transition-all duration-500 
    ...
    >
      ...
      {/* Hero Animation (SVG Data Stream) */}
      {showDataStream && (
      ...
      <div className={`relative z-10 w-full h-full ${flexRow ? 'lg:grid lg:grid-cols-12 lg:items-center lg:gap-8' : 'flex flex-col'}`}>
        ...
        {/* ICON & PULSE - Tightened Mobile Margins */}
        <div className={`${flexRow ? 'lg:col-span-1 mb-4 lg:mb-0' : 'flex justify-between items-start mb-6 md:mb-10'} w-fit lg:w-full`}>
          <div className={`p-2.5 md:p-3 rounded-xl md:rounded-2xl w-fit transition-all duration-500 ${dark ? 'bg-white/10' : 'bg-blue-50 group-hover:bg-blue-600 group-hover:text-white'}`}>
            {React.cloneElement(icon as React.ReactElement, { className: `w-5 h-5 md:w-6 md:h-6 ${dark ? 'text-white' : 'text-blue-600 group-hover:text-white transition-colors'}` })}
          </div>
          ...
          {showPulse && !flexRow && (
            <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-green-500/10 border border-green-500/20">
              ...
              <span className="font-mono text-[7px] md:text-[8px] font-bold text-green-500 uppercase tracking-tighter">Optimal</span>
            </div>
          )}
        </div>

        {/* VALUE & LABEL - Responsive Sizing and Vertical Spacing */}
        <div className={`${flexRow ? 'lg:col-span-5' : isHero ? 'mt-auto' : 'mt-4 md:mt-8'}`}>
          <div className={`font-heading text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-none ${dark ? 'text-white' : 'text-slate-900'}`}>
            {value}
          </div>
          <div className={`font-mono text-[9px] md:text-[11px] lg:text-[12px] font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] mt-2 md:mt-3 ${dark ? 'text-blue-400' : 'text-blue-600'}`}>
            {label}
          </div>
        </div>

        {/* DESCRIPTION - Reduced Top Margin on Mobile */}
        <div className={`${flexRow ? 'lg:col-span-6 lg:mt-0' : 'mt-4 md:mt-8'} lg:max-w-md`}>
          <p className={`font-sans text-xs md:text-sm lg:text-base leading-relaxed ${dark ? 'text-slate-300' : 'text-slate-500'}`}>
            {description}
          </p>
        </div>
      </div>
